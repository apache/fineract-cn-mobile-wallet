...
import org.mifos.core.domain.mappers.Mappers.depositAccountPayloadEntityMapper
...
class CreateDepositAccount(private val repository: WalletRepository) {
    ...
    suspend fun execute(depositAccountPayload: DepositAccountPayload): Result<String> {
        return try {
            val response = repository.createDepositAccount(
                depositAccountPayloadEntityMapper(depositAccountPayload)
            )
